...
def generate_json_calls(context: PromptContext, llm: Small_LLM_Model) -> None:
    """Processes all user queries and forces the LLM to output valid JSON calls."""
...
def get_function_name(context: PromptContext, llm: Small_LLM_Model, query: str) -> str:
    ...
    while len(valid_names) > 1:
        ...
        for token_id in range(len(logits)):
            ...
            if not any(name.startswith(result_name + token) for name in valid_names):
                logits[token_id] = float("-inf")
        ...
        result_name += token
        valid_names = [name for name in valid_names if name.startswith(result_name)]

    return valid_names[0]


def _decode_int(llm: Small_LLM_Model, vocab: list[str], prompt: str, remaining_text: str) -> tuple[int, str, str]:
    ...
    while True:
        ...
        for token_id in range(len(logits)):
            ...
            is_valid = all(c in "-0123456789" for c in token) or token.strip() in [',', '}']
...
def _decode_float(llm: Small_LLM_Model, vocab: list[str], prompt: str, remaining_text: str) -> tuple[float, str, str]:
...
def _decode_str(llm: Small_LLM_Model, vocab: list[str], prompt: str) -> tuple[str, str]:
    ...
    while True:
        logits = llm.get_logits_from_input_ids(input_ids)
        sorted_logits = sorted(enumerate(logits), key=lambda item: item[1], reverse=True)
...
def get_parameters(context: PromptContext, llm: Small_LLM_Model, query: str, func_name: str, partial_json: str) -> dict[str, Any]:
    ...
    prompt = (
        ...
        f"Fill this JSON to call this function: {context.schema_mapping[func_name]}:\n"
        f"{partial_json}"
    )
    ...
    for arg_name, arg_schema in params.items():
        ...
        if arg_schema['type'] == 'integer':
            val, raw_str, remaining_text = _decode_int(llm, vocab, prompt, remaining_text)
            result[arg_name] = val
            prompt += f"{raw_str},"
            
        elif arg_schema['type'] == 'number':
            val, raw_str, remaining_text = _decode_float(llm, vocab, prompt, remaining_text)
            result[arg_name] = val
            prompt += f"{raw_str},"
            
        elif arg_schema['type'] == 'string':
            val, raw_str = _decode_str(llm, vocab, prompt)
            result[arg_name] = val
            prompt += f'{raw_str}",'
